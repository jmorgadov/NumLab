...
def nl__contains__(self, indx: Instance):
    if indx.type.subtype(nl_int):
        return nl_bool.new(indx.value in self.value)
...
def nl__mul__(self, value: Instance):
    if value.type.subtype(nl_int):
        return nl_tuple.new(self.value * value.value)
...
def nl_count(self, value: Instance):
    if value.type.subtype(nl_object):
        return nl_int.new(self.value.count(value.value))
...
def nl_index(self, value: Instance, start: Instance):
    if value.type.subtype(nl_object):
        if start.type.subtype(nl_int):
            ...
            return nl_int.new(self.value.index(value.value, start_indx))
